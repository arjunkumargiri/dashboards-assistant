...
class PerformanceScalabilityTest {
  ...
  async testLargeDashboardContexts() {
    ...
    if (response.messages && response.messages.length > 0 && responseTime < 30000) {
      // 30s timeout
      console.log('✅ Large dashboard context handling working');
      this.testResults.push({
        test: 'testLargeDashboardContexts',
        status: 'passed',
        metrics: { responseTime, memoryIncrease },
      });
      return;
    }
  ...
  async testMemoryUsageValidation() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      console.log(
        `  Request ${i + 1}: Memory usage +${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`
      );
    ...
    this.testResults.push({
      test: 'testMemoryUsageValidation',
      status: 'passed',
      metrics: { totalIncrease },
    });
  }
  ...
  async testContextExtractionTiming() {
    ...
    for (const test of extractionTests) {
      ...
      if (extractionTime > maxTime) {
        console.warn(
          `  ⚠️ ${test.name} extraction time may be too slow: ${extractionTime.toFixed(2)}ms`
        );
      }
    }
  ...
  async testConcurrentUserHandling() {
    ...
    if (successRate >= 0.95 && throughput > 1) {
      // 95% success rate, >1 req/sec
    ...
    throw new Error(
      `Concurrent handling failed - Success rate: ${(successRate * 100).toFixed(
        1
  ...
  async testContextSystemUnderLoad() {
    ...
    for (const loadTest of loadTests) {
      ...
      console.log(
        `    Requests: ${requests.length} (${successfulRequests} successful, ${failedRequests} failed)`
      );
      ...
      if (successRate < 0.8) {
        // 80% minimum success rate under load
        throw new Error(
          `${loadTest.name} failed - Success rate: ${(successRate * 100).toFixed(1)}%`
        );
      }
    }
  ...
  async testCachePerformance() {
    ...
    if (cacheSpeedup > 1.2) {
      ...
      this.testResults.push({
        ...
        metrics: {
          cacheSpeedup,
          firstRequest: result1.responseTime,
          secondRequest: result2.responseTime,
        },
      });
      return;
    }
  ...
  async testDOMObserverPerformance() {
    ...
    if (avgResponseTime < 5000) {
      // Less than 5 seconds per update
  ...
  async testScalabilityLimits() {
    ...
    for (const test of scalabilityTests) {
      ...
      try {
        ...
        const result = await this.makeTimedRequest(
  ...
  generateLargeContext(vizCount, tableCount, textCount) {
    ...
    for (let i = 0; i < textCount; i++) {
      text.push(
        `Text content item ${i} with some descriptive information about the dashboard element.`
      );
    }
  ...
  printPerformanceSummary() {
    ...
    if (this.performanceMetrics.responseTime.length > 0) {
      const avgResponseTime =
        this.performanceMetrics.responseTime.reduce((sum, time) => sum + time, 0) /
    ...
    if (this.performanceMetrics.memoryUsage.length > 0) {
      const avgMemoryUsage =
        this.performanceMetrics.memoryUsage.reduce((sum, mem) => sum + mem, 0) /
    ...
    if (this.performanceMetrics.throughput > 0) {
      console.log(
        `🚀 Throughput: ${this.performanceMetrics.throughput.toFixed(2)} requests/second`
      );
    }
