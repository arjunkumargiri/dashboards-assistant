...
class ContextualChatIntegrationTest {
  ...
  async testDashboardContextExtraction() {
    ...
    const payload = {
      ...
      input: {
        ...
        content:
          'What insights can you provide about our sales performance based on this dashboard?',
        contentType: 'text',
      },
    };
    ...
    if (response.messages && response.messages.length > 0) {
      ...
      if (
        lastMessage.content.toLowerCase().includes('sales') ||
  ...
  async testVisualizationContextChat() {
    ...
    const payload = {
      ...
      input: {
        ...
        content:
          'I see some spikes in the CPU usage chart. Can you help me understand what might be causing them?',
        contentType: 'text',
      },
    };
    ...
    if (response.messages && response.messages.length > 0) {
      const lastMessage = response.messages[response.messages.length - 1];
      if (
        lastMessage.content.toLowerCase().includes('cpu') ||
  ...
  async testTableDataContextChat() {
    ...
    const tableContext = {
      ...
      navigation: {
        breadcrumbs: [{ text: 'Discover', href: '/app/discover' }],
        activeMenu: 'discover',
      },
      content: {
        visualizations: [],
    ...
    const payload = {
      ...
      input: {
        ...
        content:
          'I see 127 error entries in the logs. Can you help me identify patterns or common issues?',
        contentType: 'text',
      },
    };
  ...
  async testNavigationContextChat() {
    ...
    const payload = {
      ...
      input: {
        ...
        content:
          "I'm in the index patterns section. Can you explain how to create a new index pattern?",
        contentType: 'text',
      },
    };
    ...
    if (response.messages && response.messages.length > 0) {
      const lastMessage = response.messages[response.messages.length - 1];
      if (
        lastMessage.content.toLowerCase().includes('index pattern') ||
  ...
  async testFormControlsContextChat() {
    ...
    const payload = {
      ...
      input: {
        ...
        content:
          'I have filters set for North America and Europe regions. How can I compare their performance?',
        contentType: 'text',
      },
    };
    ...
    if (response.messages && response.messages.length > 0) {
      const lastMessage = response.messages[response.messages.length - 1];
      if (
        lastMessage.content.toLowerCase().includes('region') ||
  ...
  async testContextCaching() {
    ...
    const context = {
      ...
      content: {
        visualizations: [{ id: 'test-viz', type: 'bar_chart', title: 'Test Chart' }],
      },
    };
  ...
  async testContextualResponseProcessing() {
    ...
    if (response.messages && response.messages.length > 0) {
      const lastMessage = response.messages[response.messages.length - 1];
      if (
        lastMessage.content.toLowerCase().includes('network') &&
