...
export const ChatPage: React.FC<ChatPageProps> = (props) => {
    ...
    (content: string, isStreaming: boolean) => {
      ...
      if (content.length > 0) {
        ...
        if (!currentStreamingMessageIdRef.current && isStreaming) {
          currentStreamingMessageIdRef.current = `streaming-${Date.now()}-${Math.random()
            .toString(36)
            .substr(2, 9)}`;
          console.log(
            '🆕 CHAT PAGE: Generated new streaming message ID:',
        ...
        const existingStreamingMessage = currentMessages.find(
          (msg) => msg.messageId === currentStreamingMessageIdRef.current
        );
  ...
  const onStreamingError = useCallback(
    (error) => {
