  ...
  generateRecommendations() {
    ...
    if (failedSuites.length === 0) {
      recommendations.push(
        '🎉 All test suites passed! The contextual chat system is ready for production.'
      );
      recommendations.push(
      ...
      failedSuites.forEach((suite) => {
        if (suite.suite.includes('Integration')) {
          recommendations.push(
            '🔧 Check OpenSearch Dashboards configuration and plugin installation.'
          );
        }
        ...
        if (suite.suite.includes('Performance')) {
          recommendations.push(
            '⚡ Consider optimizing system resources or scaling infrastructure.'
          );
        }
      });
    }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p class="${
              report.summary.overallStatus === 'PASSED' ? 'status-passed' : 'status-failed'
            }">
        ...
        ${report.testSuites
          .map(
            ...
            <div class="suite ${suite.status === 'passed' ? 'suite-passed' : 'suite-failed'}">
                <h3>${suite.suite} - <span class="status-${
              suite.status
            }">${suite.status.toUpperCase()}</span></h3>
        ...
        <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; color: #666; font-size: 0.9em;">
            <p>Environment: Node.js ${report.environment.nodeVersion} on ${
      report.environment.platform
    } ${report.environment.arch}</p>
            <p>Memory Usage: ${(report.environment.memory.heapUsed / 1024 / 1024).toFixed(
              2
            )}MB heap used</p>
        </div>
    </div>
