...
export class ScreenshotService {
  ...
  static async capturePageScreenshot(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    ...
    try {
      ...
      if (result.width <= 1 || result.height <= 1) {
        ...
        throw new Error(
          `Screenshot capture failed: resulting image is only ${result.width}x${result.height} pixels`
        );
      }
      ...
      try {
        ...
        if (fallbackCanvas.width > 1 && fallbackCanvas.height > 1) {
          ...
          const fallbackResult: ScreenshotResult = {
            data: base64Data,
            mimeType: format,
            filename: `dashboard-screenshot-fallback-${Date.now()}.${
  ...
  private static hideElements(
    selectors: string[]
  ): Array<{ element: HTMLElement; originalDisplay: string }> {
   ...
   */
  private static restoreElements(
    hiddenElements: Array<{ element: HTMLElement; originalDisplay: string }>
  ) {
  ...
  static validateScreenshot(screenshot: ScreenshotResult): { valid: boolean; error?: string } {
    ...
    if (screenshot.size > MAX_SIZE) {
      return {
        valid: false,
        error: `Screenshot size (${Math.round(
          screenshot.size / 1024 / 1024
